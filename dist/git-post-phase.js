...
export async function finalizeGreenPhase(git, logger, phaseName) {
...
async function performGitPostOperations(git, logger, phaseName) {
...
async function createPullRequest(git, logger, phaseName) {
    const title = `Phase ${phaseName}`;
    const body = buildPRBody(phaseName);
    const result = await git.openPR(title, body);
    if (result.ok)
        await logger.info('Opened pull request', { phase: phaseName });
    else
...
function buildPRBody(phaseName) {
