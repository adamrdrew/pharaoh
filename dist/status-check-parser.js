...
export function parsePhaseStatus(yamlContent) {
    try {
...
function validateAndExtractStatus(parsed) {
    if (!isValidStructure(parsed))
        return buildStructureError();
    const status = parsed.phase.status;
    if (typeof status !== 'string')
        return buildStatusTypeError();
...
function isValidStructure(value) {
    ...
    if (typeof maybePhase !== 'object' || maybePhase === null)
        return false;
    return 'status' in maybePhase;
}
function buildStructureError() {
    return { ok: false, error: 'YAML missing phase.status field' };
}
function buildStatusTypeError() {
    return { ok: false, error: 'phase.status is not a string' };
}
//# sourceMappingURL=status-check-parser.js.map